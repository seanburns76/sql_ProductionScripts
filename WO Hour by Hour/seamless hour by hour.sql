...
FROM (SELECT
  ...
  s.start AS st,
  s.stop AS stp,
  DATEPART(HOUR, s.start) AS sthr,
  DATEPART(HOUR, s.stop) AS stphr,
  DATEPART(HOUR, s.stop) - DATEPART(HOUR, s.start) AS diff,
  CASE
  ...
  CASE
    WHEN DATEPART(HOUR, s.stop) = DATEPART(HOUR, s.start) THEN 0
    ELSE CASE
        WHEN DATEDIFF(D, s.start, s.stop) = 1 THEN (DATEDIFF(MINUTE, s.start, '23:59:59.999') + DATEDIFF(MINUTE, '00:00:00.000', s.stop)) / 60.0
        ELSE (DATEPART(HOUR, s.stop) - DATEPART(HOUR, DATEADD(HOUR, 1, s.start))) * 60
      END
  END AS mhr,

  60 - DATEDIFF(MINUTE, TIMEFROMPARTS(DATEPART(HOUR, s.start), 0, 0, 0, 0), CAST(s.start AS time)) AS fhr,

  s.stop,
  DATEDIFF(MINUTE, TIMEFROMPARTS(DATEPART(HOUR, s.stop), 0, 0, 0, 0), CAST(s.stop AS time)) AS lhr
FROM seamless s) y) q)
